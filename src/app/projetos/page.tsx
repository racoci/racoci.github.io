...
export default function ProjectsPage() {
  ...
  useEffect(() => {
    // Usando seu usuário padrão 'lucasracoci' (ou permitindo fallback caso não existam públicos ainda)
    const username = "lucasracoci";
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-3xl font-extrabold tracking-tight text-zinc-900 dark:text-zinc-50 font-sans mb-3">
      ...
      {!error && !loading && (
        <div className="relative">
          <input
            ...
            className="w-full px-4 py-2.5 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-lg text-zinc-900 dark:text-zinc-100 placeholder-zinc-400 dark:placeholder-zinc-600 focus:outline-none focus:ring-2 focus:ring-emerald-500/50 dark:focus:ring-emerald-400/30 focus:border-emerald-500 dark:focus:border-emerald-400 transition-all font-sans text-sm"
          />
        </div>
      )}
      ...
      {loading ? (
        ...
        <div className="p-6 text-center border border-dashed border-red-200 dark:border-red-900/50 bg-red-500/5 rounded-xl space-y-4">
          ...
          >
            Ver perfil diretamente no GitHub
          </a>
        </div>
      ) : filteredRepos.length === 0 ? (
        // Nenhum resultado encontrado
        <div className="text-center py-10 text-zinc-500 dark:text-zinc-500 font-serif">
          Nenhum projeto encontrado correspondendo ao termo "{search}".
        </div>
      ) : (
              ...
              <div className="flex items-center justify-between pt-4 mt-4 border-t border-zinc-100 dark:border-zinc-800/40 text-xs text-zinc-500 dark:text-zinc-500">
                ...
                <div className="flex items-center gap-3 font-mono">
                  <span className="flex items-center gap-1">
                    <svg className="h-3.5 w-3.5" viewBox="0 0 20 24" fill="currentColor">
